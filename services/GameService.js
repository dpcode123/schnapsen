...
module.exports = function(io, socket) {
    ...
    this.joinRoom = function(username, room) {
        try {
            ...
            if(user){
                ...
                if(true){
                    ...
                    // MOVE TYPE - EXCHANGE TRUMP CARD
                        ...
                        const jackTrumpCardName = `j-${playSession.game.trumpSuit}`;

                        // jack-trump card object
                        const jackTrumpCard = getCardByName(jackTrumpCardName);
                        
                        // remove jack in trump suit from player's hand
                        playSession.game.removeCardFromHand(jackTrumpCardName, playerIndex);

                        // push current trump card to player's hand
                        playSession.game.cardsInHand[playerIndex].push(playSession.game.trumpCard);

                        // set jack as trump card
                        playSession.game.trumpCard = jackTrumpCard;

                        // sort cards in hands; check for marriages; update clients
                        for(let i=0; i<2; i++){
                            playSession.game.sortCardsByPointsAndSuit(playSession.game.cardsInHand[i]);
                            playSession.game.marriagesInHand[i] = checkForMarriagesInHand(playSession.game.cardsInHand[i], playSession.game.trumpSuit);
                            io.to(getUsersByRoom(room)[i].id).emit('gameStateUpdateAfterTrumpExchange', new GameStateDTO(playSession.game, i));
                        }
