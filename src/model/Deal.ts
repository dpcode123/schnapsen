...
export default class Deal {
    ...
    deckClosed: boolean;
    deckClosedByPlayer: number | undefined;
    nonCloserPoints: number;
    ...
    dealRandomCardToPlayer(playerIndex: number): void {
        const randomCard = getRandomCardFromDeck(this.deck);
    ...
    dealCardsToPlayers(firstPlayer: number, numberOfCards: number): void {
        if (!this.deckClosed) {
            if (this.deck.length > 1) {
                ...
                this.dealRandomCardToPlayer(firstPlayer);
                this.cardsInHand[otherPlayer(firstPlayer)].push((this.trumpCard as Card));
                this.trumpCard = undefined;
            }
        }
    }
    ...
    addPointsToPlayer(playerIndex: number, points: number): void {
    ...
    dealOver(playerIndex: number): any {
        ...
        if (isDealOver) {
            ...
            // deck is NOT closed
                ...
                if (this.playerPoints[otherPlayer(playerIndex)] === 0) { gamePoints = 3; }

                // opponent has 1-32 points
                else if (this.playerPoints[otherPlayer(playerIndex)] > 0 && this.playerPoints[otherPlayer(playerIndex)] < 33) { gamePoints = 2; }

                // opponent has 33+ points
                else if (this.playerPoints[otherPlayer(playerIndex)] > 32 && this.playerPoints[otherPlayer(playerIndex)] < 66) { gamePoints = 1; }
            }
    ...
    // Starts next trick
    startNextTrick(lastTricksWinner: number): void {
    ...
    removeCardFromHand(cardName: string, playerIndex: number): void {
    ...
    sortCardsByPointsAndSuit = function (cardsArray: Card[]): void {
